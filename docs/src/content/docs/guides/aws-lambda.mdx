...
So widelog watches instead of waiting. A background thread polls peak RSS, and once the function
is within `memory_headroom` of its limit it emits early with `"memory_critical": true`.
...
to `0.95`. Pass `0` to turn the guard off.
