...
Every case writes to /dev/null, so the numbers include JSON serialization and a
flushed write but not terminal I/O. The "null sink" row drops serialization too,
which isolates widelog's own merge-and-build work.
"""
...
import widelog  # noqa: E402

DEVNULL = open(os.devnull, "w")

# The default sink, pointed somewhere that isn't a terminal. Mirrors emit()'s own
# json.dumps + flushed print, so the comparison against logging is apples to apples.
def devnull_sink(event: dict) -> None:
    print(json.dumps(event, default=str), file=DEVNULL, flush=True)
...
_logger.addHandler(logging.StreamHandler(DEVNULL))
...
def run(label: str, fn, sink, number: int = 20_000, repeat: int = 5) -> None:
    widelog.init(service="bench", sink=sink or devnull_sink)
